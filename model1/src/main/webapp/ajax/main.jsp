    ...
    <script>
    	...
    	function sendAsync(){
    		...
    		xhttp.onload = function(){
    			...
    			// 서버가 보내온 데이터는 무조건 문자열이기 때문에 아래의 형식이 마치 자바스크립트의 객체 리터럴로 착각될 수 없다.
    			// 결론 객체 아니다!!
    			// 객체가 아니므로, 속성이라는 것도 존재할 수 없다. 즉, 객체에 점찍고 접근불가!
    			// {"name": "동훈","email": "google"}
    			// 해결책? 어떤 문자열이 JSON의 표기법을 준수하여 작성되어 있다면, 자바스크립트는 내장 객체인 JSON 내장객체를 이용하여
    			// 문자열을 해석하여 실제 자바스크립트 객체 리터럴로 전환해줄수 있다....
    			...
    			// 서버가 보내온 데이터가 다행히도, JSON 표기법을 준수했으므로, 지금부터는 객체화 시켜서 화면에 렌더링 해보자
    			console.log("서버로부터 받은 데이터는", xhttp.responseText);
